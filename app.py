...
df = pd.read_csv('./Stocks/aa.us.txt')
...
# Wehn scaling remember! You normalize both test and train data
# with respect to training data because you are not
# supposed to have access to test data
...
# You normalize the lsat bit of remaining data
	...
	def reset_indicies(self):
		for b in range(self._batch_size):
			self._cursor[b] = np.random.randint(
				0, min((b+1)*self.segments, self._prices_length-1)
			)
...
drop_lstm_cells = [tf.contrib.rnn.DropoutWrapper(
	lstm, input_keep_prob=1.0, output_keep_prob=1.0, state_keep_prob=1.0-dropout
...
print('Definining prediction related TF functions')
...
sample_outputs, sample_state = tf.nn.dynamic_rnn(
	multi_cell, tf.expand_dims(sample_inputs, 0),
...
loss_nondecrease_threshold = 2 # If the test error hasn't increased in
							   # this many steps, decrease learning rate
